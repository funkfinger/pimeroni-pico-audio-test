...
// - Decay:   number of milliseconds it takes for a note to settle to sustain
// volume
// - Sustain: percentage of full volume that the note sustains at (duration
// implied by other factors)
// - Release: number of milliseconds it takes for a note to reduce to zero
// volume after it has ended
//
...
// |              XXXX |                   |                   | | | X    X|XX
// |                   |                   | |            X      |  XXX | | | |
// X       |     XXXXXXXXXXXXXX|XXXXXXXXXXXXXXXXXXX|                   | | X |
// |                   |X                  | |         X         | | |X | | X |
// |                   | X                 | |       X           | | | X | | X
// |                   |                   |  X                | |     X | | |
// X                | |    X              |                   | |   X | |   X |
...
struct AudioChannel {
  uint8_t waveforms =
      0;  // bitmask for enabled waveforms (see AudioWaveform enum for values)
  ...
  uint32_t adsr_end_frame =
      0;  // frame target at which the ADSR changes to the next phase
  uint32_t adsr = 0;
  int32_t adsr_step = 0;
  ADSRPhase adsr_phase = ADSRPhase::OFF;

  uint8_t wave_buf_pos = 0;  //
  int16_t wave_buffer[64];   // buffer for arbitrary waveforms. small as it's
                             // filled by user callback
  ...
  void trigger_decay() {
    ...
    adsr_step =
        (int32_t(sustain << 8) - int32_t(adsr)) / int32_t(adsr_end_frame);
  }
